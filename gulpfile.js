...
gulp.task('sass', function () {
	return gulp.src('./theme/scss/**/*.scss') // run over these files
		...
		.pipe(gulp.dest('./theme/css')) // put the css files here.
...
// 	return gulp.src('./theme/js/**/*.js') // watch these files
// 		.pipe(sourcemaps.init()) // make sourcemaps for chrome devtools
...
gulp.task('default', function() { // running `gulp` runs this task. this task sort of branches off into the others as needed
 ...
 //    });
	...
	gulp.watch('./theme/scss/**/*.scss', ['sass']); // watch sass files. if they change, run the task called "sass"
	// gulp.watch('./theme/js/**/*.js', ['js']); // watch js files. if they change, run the task called "lint"
